...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="cccccc" >  
 ...
 <% 
    ...
    String url = "qingjiaxinxi_list3.jsp?1=1"; 
    String sql =  "select * from qingjiaxinxi where jiaoshigonghao='"+request.getSession().getAttribute("username")+"' ";
...
if(request.getParameter("shifoulixiao")=="" ||request.getParameter("shifoulixiao")==null ){}else{sql=sql+" and shifoulixiao like '%"+request.getParameter("shifoulixiao")+"%'";}
 ...
 <td width='90'><a class="btn btn-info btn-small" href='<%=map.get("xiangguanpingzheng") %>' target='_blank'><img src='<%=map.get("xiangguanpingzheng") %>' width=88 height=99 border=0 /></a></td>
 ...
 <td><%=map.get("jiaoshigonghao") %></td>
    ...
    <td width="220" align="center"><a class="btn btn-info btn-small" href="qingjiaxinxi_updtlb.jsp?id=<%=map.get("id")%>">审核</a>  <a class="btn btn-info btn-small" href="qingjiaxinxi_list3.jsp?scid=<%=map.get("id") %>" onClick="return confirm('真的要删除？')">删除</a> <a class="btn btn-info btn-small" href="qingjiaxinxi_detail.jsp?id=<%=map.get("id")%>">详细</a> </td>
  </tr>
  	<%
  }
   %>
